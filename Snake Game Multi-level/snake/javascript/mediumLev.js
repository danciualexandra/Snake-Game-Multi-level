...
let color = {'snake': '','food': '','wall': ''};//culori snake , food si perete
var viewScore = "Score \n\n";
var context;
var generateFood=setInterval(food, 10000);//se apeleaza functia de food o data la 10 secunde
let snakePoint = {'x':'', 'y': ''};
let foodPoint = {'x':'', 'y': ''};
var wall=[];
let wallPoint= {'x': '','y':''};
...
function awakeMed(){
    canvas = document.getElementById('base2');
...
function drawWall(){
	...
	for(i=125;i<=325;i++){
		wallPoint['x']=i;
		wallPoint['y']=150;
		wall.push([wallPoint['x'],wallPoint['y']]);
		context.fillStyle = color['wall'];	
		context.fillRect(wallPoint['x'], wallPoint['y'], size, size);
		wallPoint['x']=i;
		wallPoint['y']=350;
		wall.push([wallPoint['x'],wallPoint['y']]);
		context.fillStyle = color['wall'];	
...
function setScore(){
    document.getElementById('scoreMed').innerText = length - 3;
}
...
function setSpeed(){
    ...
    document.getElementById('milliMed').innerText = speed;	
}
...
/*
var direction={
	'left':function position(direction){
		if(direction === 'left')
        newPosition = snakePoint['x'] - size;
		//return newPosition;
	},
    ...
    'up':function position(direction){
		if(direction === 'up')
	newPosition = snakePoint['y'] - size;
		//return newPosition;	
	},
    'down':function position(direction){
		if(direction === 'down')
			newPosition = snakePoint['y'] + size;	
		//return newPosition;	
...
function dead(){
	...
	//dead.play();
    isPaused = true;
    document.write('<center><h3>"You died"</h3><img src="deadGif.gif" usemap="#dead"><map name="dead"><area shape="rect" coords="500,556,810,590" href="medium.html"><area shape="rect" coords="1000,556,1100,590" href="index.html"></map></center> <script type= "text/javascript" src="javascript/snakeDead.js"></script>');
}
