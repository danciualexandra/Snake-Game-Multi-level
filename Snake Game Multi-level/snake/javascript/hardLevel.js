...
function generateRandomObstacles(){
	for(let i=10;i<=canvas.width;i+=70)//x
	for(let j=100;j<=canvas.height;j+=100)//y
	{var obstacle=Math.round(Math.random());
	//console.log(obstacle+"\n");
		if(obstacle==1)
		  if(i!=snakePoint['x']-size && i!=snakePoint['x']+size && j!=snakePoint['y']+size && j!=snakePoint['y']-size)
		{wallPoint={'x':i,'y':j};
